  ...
  return (
    ...
    >
      ...
      <div className="text-sm my-4 flex flex-col items-start justify-start self-stretch overflow-hidden font-medium ">
        <div
          className={`flex flex-row  justify-start ${cardType === 'bid' ? 'text-seagreen' : 'text-indianred'
            }`}
        >
          <div className="h-auto p-[2px]">
            <FaIndianRupeeSign />
          </div>
          {cardType === 'bid' ? marketSummary.totalBuyVolume : marketSummary.totalSellVolume} Cr
        </div>
        <div className="text-xs text-dimgray">{cardType === 'bid' ? marketSummary.totalBuyQty : marketSummary.totalSellQty} units</div>
      </div>
    </div>
  );
}
